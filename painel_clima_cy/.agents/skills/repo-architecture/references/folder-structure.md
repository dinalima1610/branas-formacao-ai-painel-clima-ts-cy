...
Esta regra define o padrão oficial de organização do repositório. O React não impõe uma árvore única; aqui adotamos um **híbrido** recomendado na comunidade: **agrupamento por funcionalidade (features)** para código de negócio, mais pastas por **tipo** para UI genérica, utilitários e bootstrap da app — combinação escalável citada na documentação histórica do React (“agrupar por features ou por tipo”) e em guias de projetos maduros.
