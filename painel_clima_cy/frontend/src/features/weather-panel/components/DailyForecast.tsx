...
export function DailyForecast({ daily, temperatureUnit }: DailyForecastProps) {
  return (
    <section aria-labelledby="daily-forecast-title">
      <h2 className="mb-3 font-display text-2xl text-ink" id="daily-forecast-title">
        Próximos dias
      </h2>

      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5" role="list">
        {daily.slice(0, 5).map((slot) => (
          <article className="rounded-lg bg-surface-card p-4 text-body" key={slot.date} role="listitem">
            <div className="flex items-center justify-between gap-3">
              <time className="font-medium capitalize text-ink" dateTime={slot.date}>
                {dayFormatter.format(new Date(`${slot.date}T12:00:00`))}
              </time>
              <WeatherIcon className="size-5 text-primary" iconKey={slot.conditionIconKey} label={slot.conditionLabel} />
            </div>
